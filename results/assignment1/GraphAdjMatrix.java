...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		int[][] matrixSquared = matrixSquared(adjMatrix);
		
		List<Integer> twoHopsVertices = new ArrayList<Integer>();
		for (int j = 0; j < matrixSquared.length; j++) {
			//for (int k = 0; k < matrixSquared[v][j]; k++) {
			if (matrixSquared[v][j] > 0) {
				twoHopsVertices.add(j);
			}
		}
		return twoHopsVertices;
	}
	
	private int[][] matrixSquared(int[][] original) {
		int[][] matrixSquared = new int[original.length][original.length];
		
		for (int i = 0; i < original.length; i++) {
			for (int j = 0; j < original.length; j++) {
				int sum = 0;
				for (int k = 0; k < original.length; k++) {
					sum += original[i][k] * original[k][j];
				}
				matrixSquared[i][j] = sum;
			}
		}
