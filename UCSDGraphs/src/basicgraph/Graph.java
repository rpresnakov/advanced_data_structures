...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		List<Integer> unsortedDegrees = new ArrayList();
		for (int i = 0; i < getNumVertices(); i++) {
			int neighbors = getNeighbors(i).size();
			neighbors += getInNeighbors(i).size();
			unsortedDegrees.add(neighbors);
		}
		
		Collections.sort(unsortedDegrees);
		Collections.reverse(unsortedDegrees);
		
		return unsortedDegrees;
	}
	...
	public static void main (String[] args) {
		...
		int distance2Vertex = 5;
		System.out.println("Distance2: " + graphFromFile.getDistance2(distance2Vertex));
		
		GraphAdjMatrix graphFromFileM = new GraphAdjMatrix();
		GraphLoader.loadRoadMap("data/testdata/simpletest.map", graphFromFileM);
		System.out.println(graphFromFileM);
		System.out.println("Distance2: " + graphFromFileM.getDistance2(distance2Vertex));
